# Hey, I’m Bilguuntugs 👋🔥
### 2nd-year Computer Science student • Building from zero • Shipping projects anyway
---
## 🚀 About me
I’m a university student learning software development from scratch — step by step, project by project.  
I like building tools that make life easier for students and turning pressure into progress.

---
## 🧠 What I’m focusing on right now
- Strong fundamentals (problem-solving, clean code, debugging)
- Building real projects + learning solid dev workflow (Git, teamwork, documentation)
- Consistency: *small progress every day > perfect someday*
---
## 📌 Currently learning / using
- Languages: JavaScript, Java, PhP
- Tools: Git, GitHub, Linux, VS Code, Cursor, Codex.
- Interests: student tools, productivity, WebApp.
---
## 📫 Connect with me
- LinkedIn: https://www.linkedin.com/in/bilguuntugsa
- Email: mailto:Bilguuntugs8888@gmail.com
- Portfolio: (add link)
---
⭐ If you like what I build, feel free to follow — I’m always shipping the next version.
